...
interface EditPlaylistModalProps {
  ...
  onClose: () => void;
  onSubmit: (playlistId: string, name: string, description: string, thumbnailColor: string) => Promise<void>;
}
...
const EditPlaylistModal: React.FC<EditPlaylistModalProps> = ({
  ...
  useEffect(() => {
    if (playlist) {
      ...
      setSelectedColor(playlist.thumbnail_color || '#D4AF37');
    }
  }, [playlist]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit(playlist.id, name.trim(), description.trim(), selectedColor);
      onClose();
    } catch (err) {
          ...
          <View style={styles.footer}>
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <FontAwesome5 name="save" size={16} color="#1A2C5B" />
