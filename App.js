		...
		const initializeApp = async () => {
			try {
				...
				if (notificationSuccess) {
					...
					logger.warn(
						"⚠️ Notifications initialization failed, app will continue without notifications"
					);
				}
	...
	const handleSmartQuizNotification = async (notificationData) => {
		...
		try {
			...
			if (notificationData.type === "personalized_quiz" && notificationData.quizId) {
				...
				const quiz = await SmartQuizRecommendationService.getRecommendedQuiz(
					notificationData.quizId
				);
				...
				if (quiz) {
					...
					Alert.alert("🎯 Quiz Time!", "Ready for a personalized learning session?", [
						{ text: "Maybe Later", style: "cancel" },
	...
	const handleSmartQuizDeepLink = async (quizId) => {
		try {
			...
			if (quiz) {
				...
				Alert.alert("Quiz Not Available 😔", "This personalized quiz may no longer be available.", [
	...
	const handleSharedQuiz = async (quizId) => {
		try {
			...
			if (quizData) {
				...
				Alert.alert("🎯 Quiz Challenge!", `You've been challenged to take: "${quiz.title}"`, [
					{ text: "Maybe Later", onPress: () => resetToHome(), style: "cancel" },
					{
				...
				Alert.alert("Quiz Not Available 😔", "This quiz challenge may have expired.", [
					{ text: "Go to Home", onPress: () => resetToHome() },
					{ text: "Create Quiz", onPress: () => navigateToScreen("Upload") },
				]);
			}
