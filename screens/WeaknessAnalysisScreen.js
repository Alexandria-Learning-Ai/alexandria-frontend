// screens/WeaknessAnalysisScreen.js
...
const WeaknessAnalysisScreen = ({ navigation, theme = 'light' }) => {
  const [currentWeaknesses, setCurrentWeaknesses] = useState([]);
  const [weaknessHistory, setWeaknessHistory] = useState([]);
    ...
    try {
      ...
      if (user) {
        const [weaknesses, history, pending] = await Promise.all([
          WeaknessAnalysisService.getCurrentWeaknesses(user.uid),
          WeaknessAnalysisService.getWeaknessHistory(user.uid),
          WeaknessAnalysisService.getPendingRemedialQuizzes(user.uid)
        ]);
  ...
  const PendingQuizCard = ({ quiz, index }) => (
    <Animatable.View
