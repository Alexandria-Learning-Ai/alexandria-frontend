...
export default function ExamListScreen({ navigation }) {
    ...
    // ✅ ENHANCED: Better delete handling with notification cleanup
                ...
                { 
                    ...
                    onPress: async () => {
                        try {
                            ...
                            // 🎛️ NEW: Refresh all notifications after deletion
    ...
    // ✅ ENHANCED: Better motivational messages with more variety
    const getMotivationalMessage = (daysLeft, examTitle) => {
        const messages = {
            0: [
    ...
    // ✅ NEW: Edit exam function
    const handleEditExam = (exam) => {
        // Navigate to ScheduleExamScreen with exam data for editing
        navigation.navigate('ScheduleExamScreen', { examToEdit: exam });
    };

    // ✅ NEW: Reschedule notifications
    const handleRescheduleNotifications = async (exam) => {
        try {
            const user = auth.currentUser;
            if (!user) return;

            // 🎛️ NEW: Use NotificationManager for rescheduling
            const results = await NotificationManager.scheduleAllNotifications(
                user.uid,
                {
                    id: exam.id,
                    examTitle: exam.examTitle || exam.title,
    ...
    // ✅ FIXED: Updated to handle both data structures
    const renderExamCard = ({ item }) => {
        // Handle both old and new data structures
        const examTitle = item.examTitle || item.title;
            ...
            <View style={styles.examCard}>
                ...
                >
                    <View style={styles.examCardContent}>
                        ...
                        {/* ✅ ENHANCED: Smart Study Reminder with better logic */}
                        {daysLeft > 0 && (
                        ...
                        {/* ✅ NEW: Show if reminders are enabled */}
    ...
    // ✅ NEW: Quick action for scheduling multiple exams
            ...
            [
                {
                    ...
                    onPress: () => {
                        // Could navigate to a bulk schedule screen
                        navigation.navigate('ScheduleExamScreen');
                    }
                },
                {
