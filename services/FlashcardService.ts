...
import { HierarchicalSubjectService } from './HierarchicalSubjectService';
import { QuizHistoryManager } from './QuizHistoryManager';
...
export class FlashcardService {
  static STORAGE_KEYS: StorageKeys = {
   ...
   */
  static async waitForAuth(maxWait: number = 5000): Promise<User | null> {
   ...
   */
  static async testFlashcardSystem(): Promise<boolean> {
    try {
   ...
   * @param easiness - Ease factor (default 2.5)
  ...
  /**
   * Generate flashcards from quiz mistakes with hierarchical subject classification
   */
  static async generateFlashcardsFromMistakes(
    ...
    try {
      ...
      // ✨ NEW: Get hierarchical subject classification for the quiz
      const hierarchicalClassification = HierarchicalSubjectService.classifyHierarchical(
        incorrectQuestions,
        quizResults.metadata || {}
      );

      logger.info(`🎯 Hierarchical classification: ${hierarchicalClassification.subject} → ${hierarchicalClassification.course || 'General'} (confidence: ${hierarchicalClassification.confidence})`);
      ...
      for (const question of incorrectQuestions) {
        ...
        // ✨ NEW: Enhanced flashcard with hierarchical subject structure
        const flashcard = {
          id: `mistake_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
          userId,
          source: 'quiz_mistake',

          // ✨ NEW: Hierarchical subject classification
          subject: hierarchicalClassification.subject,
          course: hierarchicalClassification.course,
          topic: hierarchicalClassification.topic,

          // Enhanced subject metadata
          subjectHierarchy: {
            subject: hierarchicalClassification.subject,
            course: hierarchicalClassification.course,
            topic: hierarchicalClassification.topic,
            confidence: hierarchicalClassification.confidence,
            source: hierarchicalClassification.source
          },

          // Visual metadata from hierarchical service
          displayInfo: HierarchicalSubjectService.getHierarchyDisplayInfo(
            hierarchicalClassification.subject,
            hierarchicalClassification.course
          ),
          ...
          reviewCount: 0,
          averageQuality: 0,
          tags: this.extractTagsFromQuestion(question, hierarchicalClassification),
  ...
  static async getFlashcardsByMode(userId, mode, filters = {}) {
    try {
      logger.info(`📚 Getting flashcards for mode: ${mode}, with filters:`, filters);
      ...
      switch (mode) {
        ...
        case 'struggling':
          ...
          break;
        case 'new':
          // Get only unreviewed cards (reviewCount === 0)
          const allCards = await this.getUserFlashcards(userId);
          flashcards = allCards.filter(card => (card.reviewCount || 0) === 0);
          break;
        case 'all':
          flashcards = await this.getUserFlashcards(userId);
          break;
        case 'review':
        default:
          flashcards = await this.getUserFlashcards(userId);
      ...
      // ✅ ENHANCED: Apply filters to ALL modes (not just 'review')
      flashcards = this.applyFlashcardFilters(flashcards, filters);

      logger.info(`✅ Returning ${flashcards.length} flashcards for mode: ${mode} after filtering`);
      return flashcards;
    } catch (error) {
  ...
  // ✅ NEW: Apply filters to flashcard array
  static applyFlashcardFilters(flashcards, filters = {}) {
    let filtered = [...flashcards];

    // Subject filter with multiple fallback fields
    if (filters.subject && filters.subject.trim()) {
      const subjectFilter = filters.subject.toLowerCase().trim();
      filtered = filtered.filter(card => {
        // Check multiple subject fields
        const cardSubject = card.subject || card.subjectHierarchy?.subject || '';
        const cardCourse = card.course || card.subjectHierarchy?.course || '';
        const cardTopic = card.topic || card.subjectHierarchy?.topic || '';

        // Match against subject, course, or topic
        return (
          cardSubject.toLowerCase().includes(subjectFilter) ||
          cardCourse.toLowerCase().includes(subjectFilter) ||
          cardTopic.toLowerCase().includes(subjectFilter)
        );
      });
    }
    ...
    if (filters.difficulty && filters.difficulty.trim()) {
      const difficultyFilter = filters.difficulty.toLowerCase().trim();
      filtered = filtered.filter(card =>
        card.difficulty && card.difficulty.toLowerCase() === difficultyFilter
      );
    }

    // Tags filter (if card has any of the specified tags)
  ...
  static async recordFlashcardReview(userId, flashcardId, quality, metadata: ReviewMetadata = {}) {
    try {
      ...
      // Update flashcard data with enhanced metadata
      const updatedFlashcard = {
        ...
        // Add metadata from enhanced version
        lastResponseTime: metadata.responseTime || null,
        lastSessionId: metadata.sessionId || null,
        lastStudyMode: metadata.studyMode || null
      };
   ...
   */
  static async getUserFlashcards(
    userId: string,
   ...
   */
  static async getLocalFlashcards(userId: string): Promise<Flashcard[]> {
    try {
   ...
   */
  static async getFlashcardsFromFirestore(userId: string): Promise<Flashcard[]> {
    try {
  ...
  // ✨ Enhanced tag extraction with hierarchical subject data
  static extractTagsFromQuestion(question, hierarchicalClassification = null) {
    const tags = [];

    // ✨ NEW: Add hierarchical subject tags
    if (hierarchicalClassification) {
      if (hierarchicalClassification.subject) {
        tags.push(hierarchicalClassification.subject.toLowerCase().replace(/\s+/g, '_'));
      }
      if (hierarchicalClassification.course) {
        tags.push(hierarchicalClassification.course.toLowerCase().replace(/\s+/g, '_'));
      }
      if (hierarchicalClassification.topic) {
        tags.push(hierarchicalClassification.topic.toLowerCase().replace(/\s+/g, '_'));
      }
    } else {
    ...
    const relevantTerms = keyTerms
      .filter(term => !['what', 'which', 'where', 'when', 'correct', 'following', 'answer', 'question'].includes(term))
      .slice(0, 3);
  ...
  // ✨ Enhanced subject breakdown with hierarchical structure
  static getSubjectBreakdown(flashcards) {
    ...
    flashcards.forEach(card => {
      // ✅ ENHANCED: Better subject extraction with multiple fallbacks
      let subject = card.subject;

      // Try alternative subject fields if main one is missing
      if (!subject || subject === '' || subject === null || subject === undefined) {
        // Try subjectHierarchy first
        if (card.subjectHierarchy?.subject) {
          subject = card.subjectHierarchy.subject;
        }
        // Try originalQuestion subject
        else if (card.originalQuestion?.subject) {
          subject = card.originalQuestion.subject;
        }
        // Try tags for subject-like terms
        else if (card.tags && Array.isArray(card.tags) && card.tags.length > 0) {
          // Use first tag as fallback subject
          subject = card.tags[0];
        }
        // Final fallback
        else {
      ...
      // Normalize subject (trim whitespace, handle empty strings)
      subject = String(subject).trim() || 'Unknown';

      // Subject level breakdown
      breakdown.bySubject[subject] = (breakdown.bySubject[subject] || 0) + 1;

      // Course level breakdown (if available)
      const course = card.course || card.subjectHierarchy?.course;
      if (course && String(course).trim()) {
        const courseKey = `${subject} → ${course}`;
        breakdown.byCourse[courseKey] = (breakdown.byCourse[courseKey] || 0) + 1;
      }

      // Topic level breakdown (if available)
      const topic = card.topic || card.subjectHierarchy?.topic;
      if (topic && String(topic).trim()) {
        const topicKey = `${subject} → ${course || 'General'} → ${topic}`;
        breakdown.byTopic[topicKey] = (breakdown.byTopic[topicKey] || 0) + 1;
      }

      // Hierarchical breakdown for dashboard display
      if (!breakdown.hierarchical[subject]) {
        breakdown.hierarchical[subject] = {
          total: 0,
          courses: {},
          displayInfo: card.displayInfo || { icon: 'book', color: '#666666' }
        };
      }
      breakdown.hierarchical[subject].total += 1;

      if (course && String(course).trim()) {
        const courseStr = String(course).trim();
        if (!breakdown.hierarchical[subject].courses[courseStr]) {
          breakdown.hierarchical[subject].courses[courseStr] = { total: 0, topics: {} };
        }
        breakdown.hierarchical[subject].courses[courseStr].total += 1;

        if (topic && String(topic).trim()) {
          const topicStr = String(topic).trim();
          breakdown.hierarchical[subject].courses[courseStr].topics[topicStr] =
            (breakdown.hierarchical[subject].courses[courseStr].topics[topicStr] || 0) + 1;
        }
      }
    });

    // ✅ Log breakdown for debugging
    logger.info('📊 Subject breakdown calculated:', {
      totalCards: flashcards.length,
      subjects: Object.keys(breakdown.bySubject),
      subjectCounts: breakdown.bySubject
    });
  ...
  static async recordStudySession(userId, sessionData) {
    try {
      ...
      logger.info(`✅ Recorded study session locally: ${sessionData.cardsStudied} cards, ${sessionData.timeSpent}s`);

      // ✅ ENHANCED: Sync flashcard session to backend analytics
      try {
        await BackendSyncService.updateFlashcardProgress({
          flashcardSetId: sessionData.flashcardSetId || 'general',
          cardsReviewed: sessionData.studied || sessionData.cardsStudied || 0,
          cardsCorrect: sessionData.correct || 0,
          timeSpent: sessionData.timeSpent || sessionData.sessionTime || 0,
          subject: sessionData.subject || 'flashcards',
          difficulty: sessionData.difficulty || 'medium',
          progress: sessionData.accuracy || 0,
          metadata: {
            session_id: sessionData.sessionId || `session_${Date.now()}`,
            mode: sessionData.mode || 'review',
            streak: sessionData.streak || 0,
            end_time: sessionData.endTime || new Date().toISOString(),
            total_cards: sessionData.totalCards || sessionData.studied || 0
          }
        });
        logger.info('✅ Flashcard session synced to backend analytics');
      } catch (syncError) {
        logger.warn('⚠️ Failed to sync flashcard session to backend (will retry later):', syncError);
  ...
  /**
   * Test-compatible method: Save a single flashcard
   */
  static async saveFlashcard(
    userId: string,
    ...
    // Use saveFlashcards method with array containing single flashcard
    const success = await this.saveFlashcards(userId, [flashcard]);

    if (success) {
      // Return the flashcard with generated metadata
      return {
  ...
  /**
   * Test-compatible method: Get all flashcards for a user
   */
  static async getFlashcards(userId: string): Promise<Flashcard[]> {
    try {
      return await this.getUserFlashcards(userId);
    } catch (error) {
  ...
  /**
   * Test-compatible method: Get flashcards due for review
   */
  static async getDueFlashcards(userId: string): Promise<Flashcard[]> {
    try {
      return await this.getFlashcardsDueForReview(userId);
    } catch (error) {
  ...
  /**
   * Test-compatible method: Update flashcard after review
   */
  static async updateFlashcardReview(
    ...
    try {
      await this.recordFlashcardReview(userId, flashcardId, quality);

      // Return updated flashcard data
      const flashcards = await this.getUserFlashcards(userId);
      const updated = flashcards.find(f => f.id === flashcardId);

      return updated || null;
    } catch (error) {
      logger.error('Error updating flashcard review:', error);
      throw error;
    }
  }

  /**
   * Test-compatible method: Delete a flashcard
   */
  static async deleteFlashcard(userId: string, flashcardId: string): Promise<boolean> {
    try {
      const flashcards = await this.getLocalFlashcards(userId);
